...
import AVLTreeList
from AVLTreeList import AVLTreeList, AVLNode
...
class AVLTreeListTester2022(unittest.TestCase):
    ...
    def verify_split(self, tree: AVLTreeList, i: int, grade, edge_case = False):
        original_list = self.in_order(tree)
        original_lower_values = original_list[0:i]
        original_higher_values = original_list[i + 1:]
        lst = tree.split(i)
        self.assertEqual(len(lst), 3, "FAIL - split should return a list of 3 arguments, not {}".format(len(lst)))
        lower_values = lst[0] ; value = lst[1] ; higher_values = lst[2]
        self.assertEqual(value, original_list[i], "FAIL - Problem with split({}), value is not correct".format(i))
        self.assertEqual(self.in_order(lower_values), original_lower_values, "FAIL - Problem with split({}), left tree".format(i))
        self.assertEqual(self.in_order(higher_values), original_higher_values, "FAIL - Problem with split({}), right tree".format(i))
        self.add_points(grade/2, False)
        if not edge_case:
            self.is_Tree_AVL(lower_values)
            self.is_Tree_AVL(higher_values)
            self.add_points(grade/2, False)
        else:
            if i == 0: self.assertTrue(lower_values.empty(), "FAIL - 'left' should be empty after split on index 0")
            else: self.assertTrue(higher_values.empty(), "FAIL - 'right' should be empty after split on last index")
            self.add_points(grade/2,False)


    def test_split_simple(self):
        T = self.create_tree([str(i) for i in range(100)], random_order=True)
        self.verify_split(tree = T, i = 50, grade = 1)
        self.add_points(0) # Only to count it as a passed test. points are given in verify_split method


    def test_split_random_indices(self):
        trees = [self.create_tree([str(i) for i in range(100)], random_order=False, insert_end=True) for j in range(10)]
        random_split_indices = random.sample(range(2,99), 10)
        for i,tree in enumerate(trees):
            self.verify_split(tree, random_split_indices[i], grade = 0.2)
        self.add_points(0) # Only to count it as a passed test. points are given in verify_split method
    ...
    def test_split_edge_cases(self):
        ...
        for i in range(length):
            self.tree.insert(i, str(i))
            self.tree_2.insert(i, str(i))
        self.verify_split(self.tree, i = 0, grade = 0.5, edge_case = True)
        self.verify_split(self.tree_2, i = length - 1, grade = 0.5, edge_case = True)
        self.add_points(0) # Only to count it as a passed test. points are given in verify_split method


    def test_split_with_retrieve_and_search(self):
        T = self.create_tree([str(i) for i in range(1000)], random_order=True)
        split_index = random.randint(200,800)
        right_first = T.retrieve(split_index + 1)
        left_last = T.retrieve(split_index - 1)
        split_val = T.retrieve(split_index)
        left, val, right = T.split(split_index)
        self.assertEqual(val, split_val, "FAIL - Problem with split({}), value is not correct".format(split_index))
        self.assertEqual(left.last(), left_last)
        self.assertEqual(right.first(), right_first)
        self.assertEqual(left.search(val), -1)
        self.assertEqual(right.search(val), -1)
        self.assertEqual(left.length() + right.length() + 1, 1000)
        self.add_points(2)
    ...
    def test_split_and_concat(self):
        ...
        in_order_before = self.in_order(T)
        split_index = random.randint(N//4, 3 * N//4)
        left, val, right = T.split(split_index)
        left.insert(split_index, val)
        left.concat(right)
        in_order_after = self.in_order(left)
        self.assertEqual(in_order_before, in_order_after, "FAIL - something is wrong with split or concat")
        self.add_points(3)
